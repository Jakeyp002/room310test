...
class Room310Handler(SimpleHTTPRequestHandler):
    ...
    def _play_shell(self, slug: str) -> None:
        ...
        is_external = game["hostType"] == "external"
        frame_markup = "" if is_external else f'<iframe id="game-frame" src="{html.escape(frame_url, quote=True)}" title="{title}" sandbox="{sandbox}" allow="fullscreen; gamepad" referrerpolicy="no-referrer" scrolling="no"></iframe>'
        loading_hidden = " hidden" if is_external else ""
        choice_markup = f'''<div id="choice" role="group" aria-label="Choose how to play">
<strong>Choose how to play</strong><p>Play on the game's original website, or try Room310's experimental online-game player.</p>
<div class="choices"><a id="play-external" href="{html.escape(frame_url, quote=True)}" target="_blank" rel="noopener noreferrer">Play Game ↗</a><button id="open-experimental" type="button">Open in Room310 (experimental)</button></div>
<small>The experimental option does not work for every online game.</small></div>''' if is_external else ""
...
#loading,#choice{{position:absolute;inset:12px;display:grid;place-items:center;align-content:center;gap:8px;padding:clamp(24px,5vw,70px);background:#10100e;text-align:center}}#loading[hidden],#choice[hidden]{{display:none}}#loading span{{width:32px;height:32px;border:2px solid #505049;border-top-color:#dfff00;border-radius:50%;animation:spin .8s linear infinite}}#loading small,#choice small{{color:#aaa89f;font:10px 'Courier New',monospace;text-transform:uppercase}}#choice>p{{max-width:580px;margin:0;color:#c4c2b9}}.choices{{width:min(100%,620px);display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:14px 0 2px}}.choices a,.choices button{{display:grid;place-items:center;min-height:58px;padding:12px 16px;text-align:center}}.choices button{{color:#f1efe7;background:#b72f25;border-color:#f05a49}}#choice>small{{color:#d7a29b}}@keyframes spin{{to{{transform:rotate(360deg)}}}}
@media(max-width:720px){{main{{grid-template-columns:1fr;grid-template-rows:48px minmax(100px,23svh) minmax(0,1fr)}}header{{grid-column:1;padding:0 10px}}aside{{padding:10px 14px;border-right:0;border-bottom:1px solid #45453f}}h1{{max-height:1.7em;margin:auto 0 6px;font-size:30px;-webkit-line-clamp:2}}p{{margin:0 0 8px;font-size:12px;-webkit-line-clamp:2}}aside button{{min-height:30px;padding:5px 9px}}section{{padding:7px}}#loading,#choice{{inset:7px}}.choices{{grid-template-columns:1fr;gap:8px;margin-top:5px}}.choices a,.choices button{{min-height:42px;padding:8px 12px}}}}@media(prefers-reduced-motion:reduce){{#loading span{{animation:none;border-color:#dfff00}}}}
...
<section><div id="viewport">{frame_markup}</div><div id="loading" role="status"{loading_hidden}><span aria-hidden="true"></span><strong>Starting game</strong><small>Loading in a secure sandbox…</small></div>{choice_markup}</section></main>
<script nonce="{nonce}">const v=document.querySelector('#viewport'),b=document.querySelector('#fullscreen'),l=document.querySelector('#loading'),c=document.querySelector('#choice'),x=document.querySelector('#open-experimental'),p=document.querySelector('#play-external');let f=document.querySelector('#game-frame');const ready=()=>{{l.hidden=true;b.hidden=!document.fullscreenEnabled}};if(f)f.addEventListener('load',ready,{{once:true}});else{{b.hidden=true;x.addEventListener('click',()=>{{c.hidden=true;l.hidden=false;x.disabled=true;f=document.createElement('iframe');f.title=document.querySelector('header strong').textContent;f.src=p.href;f.setAttribute('sandbox','{sandbox}');f.setAttribute('allow','fullscreen; gamepad');f.referrerPolicy='no-referrer';f.scrolling='no';f.addEventListener('load',ready,{{once:true}});v.append(f)}})}}b.addEventListener('click',async()=>{{try{{if(document.fullscreenElement)await document.exitFullscreen();else await v.requestFullscreen()}}catch{{}}}});document.addEventListener('fullscreenchange',()=>{{b.textContent=document.fullscreenElement?'Exit fullscreen':'Fullscreen'}})</script></body></html>"""
