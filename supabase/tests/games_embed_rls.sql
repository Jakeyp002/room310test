...
do $$
declare draft_id bigint; embed_id bigint; external_id bigint;
begin
  ...
  if (select count(*) from public.games where id in (draft_id, embed_id, external_id) and created_by = auth.uid() and updated_by = auth.uid()) <> 3 then
...
begin
  ...
  )) <> 2 then raise exception 'Anonymous draft isolation or published compatibility failed'; end if;
...
begin
  ...
  if (select count(*) from public.games where id in (
    ...
    current_setting('room310.test_game_embed')::bigint,
    current_setting('room310.test_game_external')::bigint
  )) <> 2 then raise exception 'Unapproved account can read drafts'; end if;
